...
function updateAyahInfo() {
    ...
    if (current) {
        current.textContent = currentAyah;
    }

    if (total) {
        total.textContent = surahCounts[currentSurah] || "?";
    }
}
...
async function loadReadStates(surah) {
    ...
    const res =
        await fetch(`/get_read?surah=${surah}`);

    const data = await res.json();

    Object.entries(data)
        .forEach(([ayah, is_read]) => {
...
async function loadNoteStates(surah) {
    console.log("In loadNoteStates")

    const res =
        await fetch(`/get_notes?surah=${surah}`);

    const data = await res.json();

    document.querySelectorAll(".note-dot")
        .forEach(dot => {
    ...
    document.querySelectorAll(".ayah-row")
        .forEach(card => {
            ...
            const ayah =
                String(card.dataset.ayah);

            const dot =
                card.querySelector(".note-dot");
    ...
    async (e) => {
        ...
        if (
            ...
            if (
                ...
                const res =
                    await fetch(
                        `/get_note?surah=${surah}&ayah=${ayah}`
                    );
...
function initClickableAyahs() {
    ...
    document.querySelectorAll("[data-ayah]")
        .forEach(el => {
    ...
    async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                popup.innerHTML =
                    `No definition found for <b>${word}</b>`;
